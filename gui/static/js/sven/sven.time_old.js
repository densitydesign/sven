(function(){
	...
	//			.style("opacity",0)
				...
				.enter().append("svg:rect")
					...
					.on("click", updatePosition)
						...
						nodes.filter(function(n){
							return interval.interval()(n.date) > interval.interval()(d.date);
						}).forEach(function(n){
							d3.select(n._parentNode)
								.transition()
								.attr("transform",function(g){ return "translate(" + (x(g.x) + 300) + "," + - y.range()[1] + ")" })
						})
						...
						d3.select(d._parentNode).each(function(n){ n.open = true; })

						d3.select(d._parentNode).select(".back")
							.transition()
							.attr("width",300+x.rangeBand())
						
							d3.select(d._parentNode).selectAll(".node")
								.transition()
								.delay(200)
								.attr("width",function(m){ return d.id_document == m.id_document ? 300+x.rangeBand() : 0; })
							//	.attr("height",function(m){ return d.id_document == m.id_document ? y.range()[1] : 0; })
								.attr("y",function(m){ return d.id_document == m.id_document ? 0 : 0; })
						
						
						d3.select(target)
							.data([d._parentNode.__data__])
							.append("div")
							.style("width", "320px")
							.style("height","100px")
						//	.style("background-color","#ffffff")
							.style("position","absolute")
							.style("left",function(n){ return (x(n.x) + 50) + "px"; })
							.style("top", "220px")
							.text("dadasda")
			...
			/*var hole = group.selectAll("rect.hole")
				.data(function(d){ return d.children; })
				.enter().append("svg:rect")
					.attr("class", "hole")
					.attr("y",function(d,i) { return y.range()[1] - (i+1)*elementHeight + fixHeight/2; })
					.attr("x",3)
					.attr("width",4)
					.attr("height",4)
